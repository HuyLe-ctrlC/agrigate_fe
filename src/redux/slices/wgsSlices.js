...
import wgsApi from '../../api/wgsApi';

//get all data, search, paging
export const getAllAction = createAsyncThunk('wgs/list', async (params, { rejectWithValue, getState, dispatch }) => {
    try {
        // call Api
        const response = await wgsApi.getAll(params);
...
//get data by id
export const getByIdAction = createAsyncThunk('wgs/wgs', async (id, { rejectWithValue, getState, dispatch }) => {
    try {
        // call Api
        const response = await wgsApi.getById(id);
...
//create data
export const addDataAction = createAsyncThunk('wgs/add', async (data, { rejectWithValue, getState, dispatch }) => {
    try {
        // call Api
        const response = await wgsApi.add(data);
        if (response.result) {
...
export const updateDataAction = createAsyncThunk(
    ...
    async (datas, { rejectWithValue, getState, dispatch }) => {
        ...
        try {
            // call Api
            const response = await wgsApi.update(id, data);
            if (response.result) {
                const newData = response.data[0].newData;
                const results = {
                    id: id,
...
export const statusPublishAction = createAsyncThunk(
    ...
    async (dataUpdate, { rejectWithValue, getState, dispatch }) => {
        ...
        try {
            ...
            const response = await wgsApi.status(id, body);
            if (response.result) {
                const results = {
                    msg: response.data[0].msg,
                    publish: publish,
...
//update sort by id
export const sortAction = createAsyncThunk('wgs/sort', async (dataUpdate, { rejectWithValue, getState, dispatch }) => {
    const id = dataUpdate?.id;
    ...
    try {
        ...
        const body = JSON.stringify(data);
        const response = await wgsApi.sort(id, body);
        if (response.result) {
            const results = {
                msg: response.data[0].msg,
            };
...
//delete data by id
export const deleteAction = createAsyncThunk('wgs/delete', async (id, { rejectWithValue, getState, dispatch }) => {
    try {
        // call api
...
export const selectWgs = (state) => state?.wgs;

export default wgsSlices.reducer;
