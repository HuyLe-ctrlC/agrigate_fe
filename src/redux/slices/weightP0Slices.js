...
import weightP0Api from '../../api/weightP0Api';
...
export const getByIdAction = createAsyncThunk(
    ...
    async (id, { rejectWithValue, getState, dispatch }) => {
        try {
            // call Api
            const response = await weightP0Api.getById(id);
...
export const updateDataAction = createAsyncThunk(
    ...
    async (datas, { rejectWithValue, getState, dispatch }) => {
        ...
        try {
            // call Api
            const response = await weightP0Api.update(id, data);
            if (response.result) {
                const newData = response.data[0].newData;
                const results = {
                    id: id,
...
export const statusPublishAction = createAsyncThunk(
    ...
    async (dataUpdate, { rejectWithValue, getState, dispatch }) => {
        ...
        try {
            ...
            const response = await weightP0Api.status(id, body);
            if (response.result) {
                const results = {
                    msg: response.data[0].msg,
                    publish: publish,
...
export const sortAction = createAsyncThunk(
    ...
    async (dataUpdate, { rejectWithValue, getState, dispatch }) => {
        ...
        try {
            ...
            const body = JSON.stringify(data);
            const response = await weightP0Api.sort(id, body);
            if (response.result) {
                const results = {
                    msg: response.data[0].msg,
                };
...
export const selectWeightP0 = (state) => state?.weightP0;

export default weightP0Slices.reducer;
