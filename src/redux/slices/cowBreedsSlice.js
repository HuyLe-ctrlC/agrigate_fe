...
import cowBreedsApi from '../../api/cowBreedsApi';
...
export const getAllAction = createAsyncThunk(
    ...
    async (params, { rejectWithValue, getState, dispatch }) => {
        try {
            // call Api
            const response = await cowBreedsApi.getAll(params);
...
export const getByIdAction = createAsyncThunk(
    ...
    async (id, { rejectWithValue, getState, dispatch }) => {
        try {
            // call Api
            const response = await cowBreedsApi.getById(id);
...
export const addDataAction = createAsyncThunk(
    ...
    async (data, { rejectWithValue, getState, dispatch }) => {
        try {
            // call Api
            const response = await cowBreedsApi.add(data);
            // console.log(response);
            if (response.result) {
...
export const updateDataAction = createAsyncThunk(
    ...
    async (datas, { rejectWithValue, getState, dispatch }) => {
        ...
        try {
            // call Api
            const response = await cowBreedsApi.update(id, data);
            // console.log(response);
            if (response.result) {
                const newData = response.data[0].newData;
                const results = {
...
export const statusPublishAction = createAsyncThunk(
    ...
    async (dataUpdate, { rejectWithValue, getState, dispatch }) => {
        ...
        try {
            ...
            const response = await cowBreedsApi.status(id, body);
            if (response.result) {
                const results = {
                    msg: response.data[0].msg,
                    publish: publish,
...
export const sortAction = createAsyncThunk(
    ...
    async (dataUpdate, { rejectWithValue, getState, dispatch }) => {
        ...
        try {
            ...
            const body = JSON.stringify(data);
            const response = await cowBreedsApi.sort(id, body);
            if (response.result) {
                const results = {
                    msg: response.data[0].msg,
                    sort: sort,
...
export const deleteAction = createAsyncThunk(
    ...
    async (id, { rejectWithValue, getState, dispatch }) => {
        try {
            // call api
            const response = await cowBreedsApi.delete(id);
            if (response.result) {
                const result = {
                    id,
                    msg: response.data[0].msg,
...
// init
const cowBreedsSlices = createSlice({
    name: 'cowBreeds',
    initialState: {
        data: [],
        totalPage: 0,
...
export const selectCowBreeds = (state) => state?.cowBreeds;

export default cowBreedsSlices.reducer;
